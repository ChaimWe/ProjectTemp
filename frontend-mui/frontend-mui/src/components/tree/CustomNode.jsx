...
export default function CustomNode({ data, id, selected }) {
        ...
        return (
            ...
            {showDetails && ReactDOM.createPortal(
                <div style={{
                  ...
                  <div style={{ background: '#fff', borderRadius: 12, padding: 24, minWidth: 320, minHeight: 180, boxShadow: '0 4px 24px #0002', position: 'relative' }}>
                    <button style={{ position: 'absolute', top: 8, right: 8, background: 'none', border: 'none', fontSize: 18, cursor: 'pointer' }} onClick={() => setShowDetails(false)}>&times;</button>
                    <pre style={{ fontSize: 12, color: '#222', margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>{JSON.stringify(data, null, 2)}</pre>
                  </div>
                </div>,
                document.body
            )}
    ...
    // Default node rendering (tree/graph)
    return (
        <div style={nodeStyle} title={data.name}>
            ...
            />
            <Tooltip title={data.Name || data.name} arrow>
                <div style={{ fontWeight: 'bold', fontSize: 16, textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%', color: '#fff', textShadow: darkTheme ? '0 1px 2px rgba(0,0,0,0.5)' : 'none' }}>{data.Name || data.name}</div>
            </Tooltip>
            <div style={{ fontSize: 12, textAlign: 'center', color: '#fff', margin: '2px 0' }}>{displayName}</div>
            <Handle 
                type="source" 
                position={Position.Bottom} 
                id={`source-${id}`} 
